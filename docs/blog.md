...
## [2026-03-13] 修复租户创建表单提交按钮无响应的问题
**Issue**: #11

### 问题现象
在后台管理中心点击“创建租户”按钮打开抽屉表单后，填写完信息点击“提交”按钮没有任何响应。

### 根本原因
在 Issue #7 中为了提升可维护性，我将表单逻辑抽取为独立的 `TenantForm.tsx` 组件。但在组件化过程中，犯了一个低级错误：我在父组件的抽屉里包裹了一层 `<Form>`，又在子组件内部再次渲染了 `<Form>`，导致 HTML 中出现了嵌套的 `<form><form>...</form></form>` 结构。HTML 标准不支持表单嵌套，这导致 Ant Design 的 `form.submit()` 事件代理机制失效，`onFinish` 回调无法被正确触发。

### 修复方案
1. **移除外层 Form**：在 `TenantList.tsx` 中不再包裹 `<Form>`，只保留抽屉结构。
2. **传递回调**：将 `onFinish={handleSubmit}` 作为 prop 透传给 `TenantForm`。
3. **绑定事件**：在子组件唯一的 `<Form>` 上绑定 `onFinish` prop。

### 验证
- TypeScript 类型检查通过
- 前端构建成功 (`npm run build`)
- 按钮点击后正确触发提交逻辑，调用 API 创建租户
- 2026-03-13: 批量处理完成，包含任务: 
- 2026-03-13: 批量处理完成，包含任务: 
- 2026-03-13: 批量处理完成，包含任务: 
- 2026-03-14: 批量处理完成，包含任务: 
- 2026-03-14: 批量处理完成，包含任务: 
- 2026-03-18: 批量处理完成，包含任务: 
- 2026-03-18: 批量处理完成，包含任务: 
- 2026-03-18: 批量处理完成，包含任务: 
- 2026-03-18: 批量处理完成，包含任务: 
- 2026-03-18: 批量处理完成，包含任务: 
...
- 2026-03-20: 批量处理完成，包含任务: 
- 2026-04-13: 批量处理完成，包含任务: 
- 2026-04-13: 批量处理完成，包含任务: 
- 2026-04-13: 批量处理完成，包含任务: 
- 2026-04-13: 批量处理完成，包含任务: 
- 2026-04-14: 批量处理完成，包含任务: 
- 2026-04-14: 批量处理完成，包含任务: 
- 2026-04-14: 批量处理完成，包含任务: 
- 2026-04-14: 批量处理完成，包含任务: 
- 2026-04-14: 批量处理完成，包含任务: 
...
- 2026-04-20: 批量处理完成，包含任务: 
- 2026-04-22: 批量处理完成，包含任务: 
- 2026-04-28: 批量处理完成，包含任务: 
- 2026-04-28: 批量处理完成，包含任务: 
- 2026-04-28: 批量处理完成，包含任务: 
- 2026-04-28: 批量处理完成，包含任务: 
- 2026-04-28: 批量处理完成，包含任务: 
