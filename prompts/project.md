...
### [TODO]
表头定义：即便无数据，也要渲染出包含以下列的表格：[ ] (勾选框) | 工单号 | 品牌网点 | 故障描述 | 工程师 | 留痕状态 (显示位置+相机图标) | 操作。空状态插画：当数据为空时，在表格中央显示一张简约的 SVG 维修清单插画，文案改为：“当前节点暂无工单，点击右侧按钮发起新任务”。搜索增强：在搜索框中增加 Placeholder: "搜索单号、网点、品牌、工程师姓名"。
