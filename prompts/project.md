...
1. **分公司员工生成二维码**：员工打开工单详情，点击"生成进场确认二维码"
2. **工程师扫码进场**：工程师使用微信端扫描二维码，点击"进场"，状态转为"维修中"
3. **工程师施工记录**：
   - 提交文字留言
   - 上传施工照片
   - 提交离场确认
4. **分公司验收**：分公司员工点击"验收通过"，状态转为"已完成"
...
| 层级 | 角色 | 可提权 | 可管理 | 可创建工单 | 可分配工单 | 可施工 | 可关联组织 |
|------|------|--------|--------|------------|------------|--------|------------|
...
```
分公司生成二维码 → 工程师扫码进场 → 工程师施工(留言+照片) → 工程师离场 → 分公司验收
```
...
本文档为 JobMaster 项目的核心设计文档，所有团队成员必须遵循文档中定义的规范。

### 10.1 文档清单

| 文档 | 路径 | 说明 |
|------|------|------|
| 测试用例流程 | `docs/cases.md` | 8 个核心用户场景的端到端测试流程 |
| 界面规划 | `docs/ui_design.md` | PC 端和微信端的界面设计规划 |
| API 设计 | `docs/api_design.md` | 所有 API 接口设计规范 |
| 数据库设计 | `docs/database_design.md` | 数据库结构设计（排除 IAM 用户管理） |
| 业务架构 | `docs/business_architecture.md` | 业务逻辑架构说明 |

### 10.2 文档同步要求

- 所有核心文档必须在中英文两个版本保持同步
- 任何文档修改必须同时更新 `docs/` 和 `docs/en/` 目录下的对应文件
- 文档版本号统一管理，变更记录在各自文档末尾

### 10.3 文档维护

- 文档更新需通过 Issue 跟踪
- 重大变更需经过评审后合并
- 定期检查文档一致性（使用 `/analyze` 命令）
