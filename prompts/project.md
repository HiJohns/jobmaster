...
### [TODO]
- [ ] 修复配色与对比度。Header 文本颜色：确保 Header 中的用户信息为深灰色（#666），在白色背景上清晰可见。Sidebar 激活态：选中菜单项时，背景变为蓝色，且左侧有一条 3px 的亮蓝线条，增加视觉引导。
- [ ] 处理 UUID 的业务化展示。语义化转换：如果当前租户是超级管理员，顶栏显示“系统管理后台”；如果是分店，显示具体的“分店名称”。隐藏 ID：除非在开发者调试模式，否则永远不要向用户展示那一长串 UUID。
